...
function extractPlateFromImageFile($imagePath) {
    ...
    $query = "What is the number plate of this vehicle in alphanumeric format ?  Include only aphanumeric characters in the plate.   Is a paper notice pinned on the windsceen?    JSON format element 'plate', 'notice_pinned'";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_FILES['photo'])) {
    ...
    $stmt = $conn->prepare("INSERT INTO notice (plate, noticepinned, result, uploadFile, phototime, `checksum`, `uuid`) 
                                   VALUES (?, ?, ?, ?, ?, ?, ?)");
    error_log($conn->error);
    $stmt->bind_param("sssssss", $plate, $noticepinned, $result, $uploadFile, $phototime, $checksum, $uuid);
    ...
    $stmt = $conn->prepare("UPDATE notice set plate = ?, noticepinned = ?,  result = ?, `checksum`=?, timestamp=timestamp where id=?");
    $stmt->bind_param("sssss", $plate, $noticepinned,  $result, $checksum, $id);
}
